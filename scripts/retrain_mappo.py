"""
Retrain MAPPO with updated reward (DISTANCE_SHAPING=0.3, reward_version=2.0.0).
Usage: python scripts/retrain_mappo.py
       python scripts/retrain_mappo.py --seeds 42 43 44 --n_episodes 500
       python scripts/retrain_mappo.py --run_tag v1
       python scripts/retrain_mappo.py --n_seekers 4 --run_tag v1
"""
import sys
import argparse
import warnings
...
parser.add_argument("--run_tag",     type=str, default="v1")
...
for seed in args.seeds:
    ...
    run_name = f"mappo_{args.run_tag}_seed{seed}"
    logger = EpisodeLogger(run_name, "runs/")
    train(env, n_episodes=args.n_episodes, seed=seed, logger=logger)
    logger.close()
    print(f"seed={seed} done — logs: runs/{run_name}.csv  checkpoint: checkpoints/mappo_seed{seed}/ep{args.n_episodes}/")
